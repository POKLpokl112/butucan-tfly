...
public class RelativeDataManager {
	...
	public void putData(final String fullName, final String abbrName, final boolean isCheck) throws SQLException {
		...
		update.execute(update.new SqlExecutor() {
			...
			protected void doExecute() throws SQLException {
				ps = prepareStatement("select id,data,comment from " + update.getTable() + " where id =? or data=?");
				...
				if (rs.next()) {
					...
					if (id.equals(fullName) && data.equals(abbrName)) {
						return;
					}
	...
	protected void update(final String id, final String comment) throws SQLException {
		update.execute(update.new SqlExecutor() {

			@Override
			protected void doExecute() throws SQLException {
				ps = prepareStatement("update " + update.getTable() + " set comment=? where id =?");
...
// }
//
// NoSqlUpdater update = new NoSqlUpdater("pevc",
// "relativelycompanyname.relativelycompanyname", false);
//
// public void putData(final String fullName, final String abbrName) throws
// SQLException {
// putData(fullName, abbrName, false);
// }
//
// public void putData(final String fullName, final String abbrName, final
// boolean isCheck) throws SQLException {
// // check
// Assert.assertTrue(Util.notEmpty(fullName) && Util.notEmpty(abbrName));
//
// update.execute(update.new SqlExecutor() {
...
// @Override
// protected void doExecute() throws SQLException {
// ps = prepareStatement("select id,data,status from " + update.getTable() + "
// where id =? or data=?");
// ps.setString(1, fullName);
// ps.setString(2, abbrName);
// rs = ps.executeQuery();
...
//
// if (rs.next()) {
// Util.getLogger(this).warn("more than one row!-----" + fullName + "=" +
// abbrName);
// return;
// }
//
...
// }
//
// public void putCheckedData(final String fullName, final String abbrName)
// throws SQLException {
// putData(fullName, abbrName, true);
// }
//
// protected void update(final String fullName, final String abbrName) throws
// SQLException {
// update.execute(update.new SqlExecutor() {
//
// @Override
...
// }
//
// protected void insert(final String fullName, final String abbrName, final
// boolean isCheck) throws SQLException {
// update.execute(update.new SqlExecutor() {
//
// @Override
// protected void doExecute() throws SQLException {
// ps = prepareStatement("insert into " + update.getTable() + " (id,data,status)
// values (?,?,?)");
// ps.setString(1, fullName);
// ps.setString(2, abbrName);
// ps.setString(3, isCheck ? "OK" : "");
